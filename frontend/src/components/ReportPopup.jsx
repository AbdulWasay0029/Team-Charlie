...
export default function ReportPopup({ report, onVote, hasVoted }) {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'pending':
        return <span className="bg-slate-800 border border-slate-700 text-slate-400 text-[9px] px-2.5 py-0.5 rounded-full font-bold font-mono uppercase tracking-wider">Verification Pending</span>;
      case 'live':
        return <span className="bg-emerald-950/40 border border-emerald-900/30 text-emerald-400 text-[9px] px-2.5 py-0.5 rounded-full font-bold font-mono uppercase tracking-wider animate-pulse">AI Verified Live</span>;
      case 'rejected':
        return <span className="bg-rose-955/20 border border-rose-900/40 text-rose-455 text-[9px] px-2.5 py-0.5 rounded-full font-bold font-mono uppercase tracking-wider">AI Rejected</span>;
      case 'resolved':
        return <span className="bg-teal-950/40 border border-teal-900/30 text-teal-400 text-[9px] px-2.5 py-0.5 rounded-full font-bold font-mono uppercase tracking-wider">Resolved</span>;
      default:
  ...
  return (
    <div className="w-80 flex flex-col max-h-[420px] text-slate-100 font-body bg-slate-900 rounded-2xl overflow-hidden shadow-2xl border border-slate-800/80">
      {/* Photo Header */}
      <div className="relative h-40 w-full overflow-hidden bg-slate-950 group shrink-0">
        <img 
        ...
        {totalPhotos > 1 && (
          <div className="absolute top-2 right-2 z-10 bg-slate-950/80 backdrop-blur-md text-slate-200 text-[9px] px-2 py-0.5 rounded-full font-bold font-mono border border-slate-800/50">
        ...
        {totalPhotos > 1 && (
          <>
            <button
              ...
              className="absolute left-1.5 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/85 text-white w-6 h-6 rounded-full flex items-center justify-center z-10 transition cursor-pointer text-xs font-bold"
            >
              ◀
            </button>
            <button
              ...
              className="absolute right-1.5 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/85 text-white w-6 h-6 rounded-full flex items-center justify-center z-10 transition cursor-pointer text-xs font-bold"
            >
              ▶
            </button>
          </>
      ...
      {/* Details Container */}
      <div className="p-4 space-y-3.5 overflow-y-auto flex-1 bg-slate-900 border-t border-slate-800/80">
        ...
        {/* Info Grid */}
        <div className="grid grid-cols-2 gap-2 text-[9px] bg-slate-950 border border-slate-850 p-2.5 rounded-xl font-mono text-slate-400">
